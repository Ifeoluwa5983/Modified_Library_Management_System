...
public interface BookRepository extends JpaRepository<Book, Integer> {
    ...
    default Book saveBook(Book book) throws BookException {
        Book savedBook = null;
        if(isBookValid(book)){
            savedBook = save(book);
        }
    ...
    private boolean isBookValid(Book book) throws BookException {
        if(!bookHasLibrary(book)){
            throw new BookException("Where is the registered library for this book?");
        }
        if(!bookHasAuthor(book)){
            throw new BookException("Who wrote this book?");
        }
        if(!bookHasTitle(book)){
            throw new BookException("Don't be silly! where is the title?");
        }
        return true;
    }
